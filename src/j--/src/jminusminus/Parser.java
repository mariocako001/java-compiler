...
public class Parser {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        // now has implements option
        ArrayList<TypeName> implementsTypeList = new ArrayList<TypeName>();
        
       	if (have(IMPLEMENTS)){
       		implementsTypeList.add(qualifiedIdentifier());
       		while(have(COMMA)){
       			implementsTypeList.add(qualifiedIdentifier());
       		}
       	}
        return new JClassDeclaration(line, mods, name, superClass, implementsTypeList ,classBody());
    }
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods){
    	...
    	mustBe(IDENTIFIER);
    	String name = scanner.previousToken().image();
    	ArrayList<TypeName> extendedTypeList = new ArrayList<TypeName>();
    	if (have(EXTENDS)){
    		extendedTypeList.add(qualifiedIdentifier());
    		while (have(COMMA)){
    			extendedTypeList.add(qualifiedIdentifier());
    		}
    	}
    	return new JInterfaceDeclaration(line, mods, name, extendedTypeList, interfaceBody());
    }
    ...
    private ArrayList<JMember> classBody() {
        ...
        while (!see(RCURLY) && !see(EOF)) {
        	...
        	if (have(STATIC) && see(LCURLY)){
        		// if we see STATIC LCURLY
        		members.add(new JStaticBlock(scanner.token().line(), block()));
        	} else if (see(LCURLY)){
        	 	// if we see LCURLY means its just an instance block
        	 	members.add(new JInstanceBlock(scanner.token().line(), block()));
        	} else{
        		scanner.returnToPosition();
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (have(VOID)) {
                ...
                if(have(THROWS)){
            		qualIdentifier.add(qualifiedIdentifier());
            		while(have(COMMA)){
            			qualIdentifier.add(qualifiedIdentifier());
            		}
            	}
                ...
                memberDecl = new JMethodDeclaration(line, mods, name, type,
                        params, qualIdentifier, body);
        } else {
        	type = type();
            if (seeIdentLParen()) {
	            // Non void method (type method)
	            mustBe(IDENTIFIER);
	            String name = scanner.previousToken().image();
	            ArrayList<JFormalParameter> params = formalParameters();
	            ArrayList<TypeName> qualIdentifier = new ArrayList<TypeName>();
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            JBlock body = block();
            memberDecl = new JConstructorDeclaration(line, mods, name, params, qualIdentifier,
                    body);
        } else {
            Type type = null;
            if (have(VOID)) {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            // have not figured [FINAL] support yet
            ...
            if (isEnhanced()) {
                ...
                mustBe(RPAREN);
                body = block();
                return new JEnhancedForStatementOp(line,formalParameter,expression,body);
            }

            // forInit
            if (seeBasicType() || seeReferenceType()) {
                forInit.add(new JForInitDeclarationOp(line, variableDeclarators(type())));
            } else {
            ...
            mustBe(RPAREN);
            body = block();
            return new JForStatementOp(line, forInit, expression, forUpdate, body);
        } else if(have(TRY)){
...
// Function to check if for loop is enhanced or not. Usually if we see a type, variable name and then colon
// it means we have an enhanced for loop
// need to use scanner position to look down the line and then return again so we can evaluate it
  ...
  private boolean isEnhanced() {
     ...
     * </pre>
     * 
     * @return an AST for an conditionalExpression or ternary expression.
     */
    ...
    private JExpression conditionalExpression(){
    	...
    	if(have(TERNARY)){
    		JExpression truePart = assignmentExpression();
    		mustBe(COLON);
    		JExpression falsePart = conditionalExpression(); 
    		return new JConditionalExpressionOp(line, lhs, truePart, falsePart);
    	} else {
    		return lhs;
    	}
    }
	 ...
	 * Level 11
	 * 
     * Parse an conditional or expression.
     * 
     * <pre>
     ...
     * </pre>
     * 
     * @return an AST for an conditional or expression.
     */
...
private JExpression conditionalOrExpression() {
        ...
        if (have(LOR)) {
            return new JLogicalOrOp(line, lhs, conditionalAndExpression());
        } else {
            return lhs;
        }
    }
	...
	/**
     * Parse a inclusiveOrExpression expression.
     * 
     * <pre>
     *   inclusiveOrExpression ::= inclusiveORExpression  // level 9
	...
	/**
     * Parse a exclusiveOrExpression expression.
     * 
     * <pre>
	...
	/**
     * Parse a andExpression expression.
     * 
     * <pre>
    ...
    private JExpression shiftExpression() {
        ...
        if (have(SHL)) {
            ...
            return new JLogicalRightShiftOp(line, lhs, additiveExpression());
        } else {
            return lhs;
        }
    }
