...
class Scanner {
	...
	public TokenInfo getNextToken() {
		...
		while (moreWhiteSpace) {
			...
			if (ch == '/') {
				...
				if (ch == '/') {
					...
					if (ch == EOFCH) {
						reportScannerError("End of multi-line comment '*/' is not supported.");
					}
				} else if (ch == '=') {
					nextCh();
		...
		// could be a DOT, DOUBLE or FLOAT
		case '.':
			...
			// if its number i check if float or double
			// else return dot
			...
			if (isDigit(ch)) {
				...
				if (ch == 'e' || ch == 'E') {
					...
					// .(0-9)e[f|F] or .(0-9)e[d|D] not allowed
					if (ch == 'f' || ch == 'F' || ch == 'd' || ch == 'D') {
						buffer.append(ch);
						reportScannerError("Invalid Number: %s", buffer.toString());
						nextCh();
						return getNextToken();
					}
		...
		case '0':
			...
			if (ch == '.') {
				...
				// to reach here it means i has seen a number
				// after the DOT, otherwise it has returned
				// double or float

				// if i reach an e|E
				// then optional +|-
				// then must see digit
				// then if d|D return double
				// if f|F return float
				// else return double again
		...
		case '9':
            ...
            if (ch == 'e' || ch == 'E'){
                ...
                // could be a double or float with a dot
                buffer.append(ch);
                nextCh();
                // case 0.
                // works for double
